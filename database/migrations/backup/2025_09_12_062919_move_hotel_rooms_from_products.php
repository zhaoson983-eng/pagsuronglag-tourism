...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (Schema::hasColumn('products', 'type')) {
            // Move hotel rooms from products to hotel_rooms
            $hotelRooms = DB::table('products')
                ->where('type', 'hotel_room')
                ->get();

            foreach ($hotelRooms as $product) {
                $amenities = [];
            ...
            // Remove hotel rooms from products
            DB::table('products')
                ->where('type', 'hotel_room')
                ->delete();
        }
    ...
    public function down(): void
    {
        // Check if the type column exists in the products table
        if (Schema::hasColumn('products', 'type')) {
            // Move hotel rooms back to products
            $hotelRooms = DB::table('hotel_rooms')->get();
