...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="px-4 py-6 bg-white">
        ...
        <div class="grid grid-cols-4 gap-4 md:max-w-2xl md:mx-auto">
            ...
            <a href="{{ route('customer.attractions') }}" class="flex flex-col items-center p-4 rounded-xl bg-blue-100 border-2 border-blue-300 transition-colors">
                ...
                <span class="text-xs md:text-sm font-medium text-blue-700 text-center leading-tight">Attractions</span>
            </a>
        </div>
    ...
    <div class="px-4 py-2 max-w-2xl mx-auto">
        ...
        <div id="no-content" class="text-center py-12 hidden">
            ...
            <p class="text-gray-500 text-lg">No attractions available at the moment</p>
            <p class="text-gray-400 text-sm">Check back later for amazing places to visit!</p>
        </div>
    </div>
</div>
@endsection
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Attractions page loaded, starting feed load...');
    loadFeedData();
            ...
            <div class="flex items-center justify-between mb-3">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        ${item.description ? `<p class="text-xs text-gray-400">${item.description.substring(0, 50)}${item.description.length > 50 ? '...' : ''}</p>` : ''}
                    </div>
                </div>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    <button class="flex items-center space-x-1 transition-colors like-btn" 
                            onclick="toggleLike('attraction', ${item.id})" 
                    ...
                    <button class="flex items-center space-x-1 transition-colors" onclick="showRating('attraction', ${item.id})">
                        <i class="fas fa-star text-yellow-400"></i>
                        <span class="text-sm text-gray-600">${(item.rating || 0).toFixed(1)} (${item.rating_count || 0})</span>
                    </button>
                    <button class="flex items-center space-x-1 text-gray-600 hover:text-blue-600 transition-colors" onclick="showComments('attraction', ${item.id})">
...
function toggleLike(type, id) {
    let route = `/tourist-spots/${id}/like`;
    ...
    .then(data => {
        if (data.success) {
            ...
            if (button) {
                ...
                // Update like count
                if (countSpan) {
                    countSpan.textContent = data.like_count || data.likes_count || data.total_likes || 0;
                }
...
function submitRating(event, type, id) {
    ...
    let route = `/tourist-spots/${id}/rate`;
    ...
    .then(data => {
        if (data.success) {
            ...
            alert(data.error || data.message || 'Error submitting rating');
        }
    })
    .catch(error => {
...
function loadComments(type, id) {
    let route = `/tourist-spots/${id}/comments`;
...
function submitComment(event, type, id) {
    ...
    let route = `/tourist-spots/${id}/comment`;
    ...
    .then(data => {
        if (data.success) {
            ...
            alert(data.error || data.message || 'Error submitting comment');
        }
    })
    .catch(error => {
    ...
    if (confirm('Are you sure you want to delete this comment?')) {
        ...
        .then(data => {
            if (data.success) {
                ...
                alert(data.error || data.message || 'Error deleting comment');
            }
        })
        .catch(error => {
